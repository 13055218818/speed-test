...
#import <YYLabel.h>
@interface CSMineCourseCell ()

@property (nonatomic, strong)CSCourseBasicModel * baseModel;
...
@implementation CSMineCourseCell
...
- (void)mockData{
    ...
    self.thumbnailImageView.image = [UIImage imageNamed:@"cs_home_course"];
    self.titleLabel.text = @"T_T";
    self.authorLabel.text = @"来了老弟";
    self.priceLabel.text = @"免费";
    self.courceLabel.text = @"共2节";
    self.studyLabel.text = @"共120人学习";
    self.typeLabel1.text = @"吉他教学";
    self.typeLabel2.hidden = YES;
}

- (void)configModel:(CSCourseBasicModel*)model{
    
    self.baseModel = model;
    
    self.baseModel = model;
    if (![NSString isNilOrEmpty:model.image]) {
        [self.thumbnailImageView sd_setImageWithURL:[NSURL URLWithString:model.image] placeholderImage:[UIImage imageNamed:@"cs_home_course"]];
    }
    
    self.titleLabel.text = model.title;
    self.authorLabel.text = model.subject_name;
