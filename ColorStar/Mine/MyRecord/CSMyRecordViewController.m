...
NSString * const CSMineCourseCellReuseIdentifier1 = @"CSMineCourseCellReuseIdentifier1";
NSString * const CSEmptyDataCellReuseIdentifier2 = @"CSEmptyDataCellReuseIdentifier2";


@interface CSMyRecordViewController ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedString(@"学习记录", nil);
    self.firstLoad = YES;
}
...
- (void)loadData{
    ...
    [[CSNetworkManager sharedManager] quaryBrowseListSuccessComplete:^(CSNetResponseModel * response) {
...
- (void)setupTableView{
    ...
    [self.tableView registerClass:[CSMineCourseCell class] forCellReuseIdentifier:CSMineCourseCellReuseIdentifier1];
    [self.tableView registerClass:[CSEmptyDataCell class] forCellReuseIdentifier:CSEmptyDataCellReuseIdentifier2];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.list.count > 0) {
        CSMineCourseCell * cell = [tableView dequeueReusableCellWithIdentifier:CSMineCourseCellReuseIdentifier1 forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        CSCourseBasicModel * baseModel = self.list[indexPath.row];
        [cell configModel:baseModel];
        return cell;
    }else{
        CSEmptyDataCell * emptyCell = [tableView dequeueReusableCellWithIdentifier:CSEmptyDataCellReuseIdentifier2 forIndexPath:indexPath];
        return emptyCell;
    }    
