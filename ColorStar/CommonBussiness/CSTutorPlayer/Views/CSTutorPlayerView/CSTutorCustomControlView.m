...
@interface CSTutorCustomControlView () <ZFSliderViewDelegate>
...
@implementation CSTutorCustomControlView
...
- (void)layoutSubviews {
    ...
    self.backBtn.frame = CGRectMake(min_x, min_y, 26, 26);
    self.backBtn.zf_centerY = self.topToolView.zf_centerY;
    self.backBtn.hidden = self.player.isFullScreen;
    ...
    self.playOrPauseBtn.frame = CGRectMake(min_x, min_y, 22, 22);
    ...
    min_x = self.playOrPauseBtn.zf_right + 8;
    min_y = 0;
...
- (void)videoPlayer:(ZFPlayerController *)videoPlayer currentTime:(NSTimeInterval)currentTime totalTime:(NSTimeInterval)totalTime {
    if (!self.slider.isdragging) {
        ...
        self.totalTimeLabel.text = [NSString stringWithFormat:@"%@/%@",currentTimeString,totalTimeString];
...
- (UIButton *)playOrPauseBtn {
    if (!_playOrPauseBtn) {
        ...
        [_playOrPauseBtn setImage:LoadImage(@"cs_tutor_player_icon_start") forState:UIControlStateNormal];
        [_playOrPauseBtn setImage:LoadImage(@"cs_tutor_player_icon_stop") forState:UIControlStateSelected];
    }
